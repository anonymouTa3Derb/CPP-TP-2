...
// de départ et d'arrivée correspondent à celles recherchées
{
    ...
    for(unsigned int i=0; i<trajets.Longueur(); ++i)
    {
        ...
        if(strcmp(t->RecupDepart(), villeD) == 0 && strcmp(t->RecupArrivee(), villeA) == 0)
        {
            cout << "+ Parcours : ";
            t->Afficher();
            trouve = true;
...
// Recherche récursive en profondeur avec retour sur trace
{
    ...
    for(unsigned int i=0; i<niveauMax; ++i)
    {
        ...
        if(strcmp(candidat->RecupDepart(), depart) != 0)
        {
        ...
        if(!utilise)
        {
            ...
            if(strcmp(candidat->RecupArrivee(), arrivee) == 0)
            {
                trouve = true;
                cout << "+ Parcours : ";
                for(unsigned int j=0; j<=niveau; ++j)
            ...
            explorerEnProfondeur(parcours, candidat->RecupArrivee(), arrivee, niveauMax, trouve, niveau+1);
        }
    }
